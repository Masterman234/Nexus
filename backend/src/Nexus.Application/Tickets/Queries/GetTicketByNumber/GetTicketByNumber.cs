...
namespace Nexus.Application.Tickets.Queries.GetTicketByNumber;

public static class GetTicketByNumber
{
    public record Query(Guid WorkspaceId, int Number) : IRequest<Result<TicketDetailResponse>>;
        ...
        : IRequestHandler<Query, Result<TicketDetailResponse>>
    {
        public async Task<Result<TicketDetailResponse>> Handle(Query request, CancellationToken cancellationToken)
        {
            var ticketResponse = await (from t in dbContext.Tickets
                                        ...
                                        where t.WorkspaceId == request.WorkspaceId && t.Number == request.Number
...
public record TicketDetailResponse(
    ...
    List<TicketStatusChangeResponse> History);
